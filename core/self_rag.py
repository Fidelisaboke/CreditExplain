...
class SelfRAG:
    ...
    def __init__(
        ...
        self.provenance_logger = ProvenanceLogger(
            AuditConfig(log_to_console=True)
        )
        ...
        logger.info("SelfRAG system initialized with adaptive retrieval and self-reflection")
    ...
    def run(self, query: str, case_id: Optional[str] = None) -> Dict:
        ...
        try:
            ...
            retrieval_notes = retrieval_decision.get("notes", "")
            ...
            if not should_retrieve:
                ...
                audit_path = self.provenance_logger.write_audit(
                    run_id, query, [], generated_response, 
                    provenance_meta, processing_time, case_id
                )
            ...
            logger.info(f"Generating and scoring answers for {len(top_passages)} top passages...")
            candidate_results = []
            ...
            if not candidate_results:
                ...
                return self._handle_processing_failure(run_id, query, top_passages, start_time, case_id)
            ...
            if best_score_components.get("issup", 0.0) < self.fully_supported_threshold:
                ...
                return self._handle_insufficient_support(
                    run_id, query, top_passages, candidate_results, 
                    best_score_components, start_time, case_id
                )
            ...
            audit_path = self.provenance_logger.write_audit(
                run_id, query, top_passages, final_answer, 
                provenance_meta, processing_time, case_id
            )
            ...
            return {
                ...
                "audit_path": audit_path,  # Changed from audit_id to audit_path
                "retrieval_performed": True,
    ...
    def _handle_empty_retrieval(self, run_id: str, query: str, start_time: float, case_id: str) -> Dict:
