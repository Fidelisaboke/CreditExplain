...
def normalize_text(text: str, redact_pii: bool = True, normalize_regulatory: bool = True) -> str:
    """
...
def normalize_rules(rules: List[Dict], in_place: bool = True, **kwargs) -> List[Dict]:
    ...
    if in_place:
        ...
        return [{**rule, 'text': normalize_text(rule.get('text', ''), **kwargs)} 
                for rule in rules]
...
def create_normalization_report(text: str) -> Dict[str, Any]:
    ...
    return {
        ...
        'normalized_sample': normalized_text[:200] + '...' if len(normalized_text) > 200 else normalized_text,
    }
